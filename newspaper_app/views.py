...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["previous_post"] = (
            Post.objects.filter(
                status="active", published_at__isnull=False, id__lte=obj.id
            )
            .order_by("-id")
            .first()
        )
        ...
        context["next_post"] = (
            Post.objects.filter(
                status="active", published_at__isnull=False, id__gte=obj.id
            )
            .order_by("id")
            .first()
        )
...
class NewsletterView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if is_ajax == "XMLHttpRequest":   # if ajax request ho vane form save gara
            form = self.form_class(request.POST)
            if form.is_valid():
                form.save()
                return JsonResponse(
